...
import { User, ChevronDown, Menu, X, BookOpen, Home, Shield } from "lucide-react";
    ...
    <header className="bg-card shadow-md sticky top-0 z-50 border-b border-border">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center space-x-3 group">
          ...
          <span className="text-xl font-bold font-heading text-foreground group-hover:text-primary transition-colors">Smart Abhyas</span>
        </Link>

        {/* Desktop Navigation */}
        <nav className="hidden md:flex items-center space-x-1">
          {navLinks.map(link => (
            <Link key={link.name} to={link.href} className="px-4 py-2 rounded-lg text-muted-foreground hover:text-primary hover:bg-primary/5 transition-all duration-200 font-medium">
              {link.name}
            </Link>
          ))}
